...
def decideWhoIsFirst(s):
    ...
    if True:  # int(input("Who plays first? 1-MC / anything else-you : ")) == 1:
        s.playTurn = COMPUTER
    else:
        s.playTurn = HUMAN
    return s.playTurn
...
def check_leagal_move(s, row_num):
    # Reads, enforces legality and executes the user's move.
...
def scan_col(s):
    ...
    for i in range(0, rows):
        if check_leagal_move(s, i):
            new_s = cpy(s)
            makeMove(new_s, i)
            temp = {i: (play_rand_games(new_s))}
            dict.update(temp)
    return dict


def play_rand_games(new_s):
    saved_s = cpy(new_s)
    comp_count = 0
    for i in range(0, 100):  # This loops takes about 15 seconds on my computer
        while not isFinished(new_s):
            if isHumTurn(new_s):  # The simple agent plays "Human"
                inputRandom(new_s)
            else:
                inputRandom(new_s)  # The MC agent plays "Computer"
                #printState(new_s)
        if value(new_s) == 10 ** 20:  # the MC Agent won
            comp_count += 1
        #print("Start another game")
        new_s = cpy(saved_s)
    return (comp_count / (i + 1))
